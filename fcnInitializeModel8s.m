...
filters = single(bilinear_u(4, 1, 40)) ;
...
net.addLayer('skip3', ...
     dagnn.Conv('size', [1 1 256 40]), ...
     'x17', 'x43', {'skip3f','skip3b'});

f = net.getParamIndex('skip3f') ;
net.params(f).value = zeros(1, 1, 256, 40, 'single') ;
net.params(f).learningRate = 0.01 ;
net.params(f).weightDecay = 1 ;

f = net.getParamIndex('skip3b') ;
net.params(f).value = zeros(1, 1, 40, 'single') ;
...
filters = single(bilinear_u(16, 40, 40)) ;

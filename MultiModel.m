function net = MultiModel(varargin)
...
net.addLayer('conv22_d',net2.layers(idx).block,{'d8'},{'d10'},{'conv22f_d','conv22b_d'});
...
net.addLayer('dropout1_rgb', net1.layers(idx).block,{'rgb35'},{'rgb35a'}, {});
net.addLayer('dropout1_d', net2.layers(idx).block,{'d35'},{'d35a'}, {});

% fc7
idx = net1.getLayerIndex('fc7');
net.addLayer('fc7_rgb',net1.layers(idx).block,{'rgb35a'},{'rgb36'},{'fc7f_rgb','fc7b_rgb'});
net.addLayer('fc7_d',net2.layers(idx).block,{'d35a'},{'d36'},{'fc7f_d','fc7b_d'});
...
reluBlock_rgb = dagnn.ReLU() ;
reluBlock_d = dagnn.ReLU() ;
net.addLayer('relu7_rgb', reluBlock_rgb, {'rgb36'}, {'rgb37'}, {});
net.addLayer('relu7_d', reluBlock_d, {'d36'}, {'d37'}, {});

% Dropout2 Layer
idx = net1.getLayerIndex('dropout2');
net.addLayer('dropout2_rgb', net1.layers(idx).block,{'rgb37'},{'rgb38'}, {});
net.addLayer('dropout2_d', net2.layers(idx).block,{'d37'},{'d38'}, {});
...
% fc8
idx = net1.getLayerIndex('fc8');
net.addLayer('fc8_rgb',net1.layers(idx).block,{'rgb38'},{'rgb39'},{'fc8f_rgb','fc8b_rgb'});
net.addLayer('fc8_d',net2.layers(idx).block,{'d38'},{'d39'},{'fc8f_d','fc8b_d'});

f1 = net.getParamIndex('fc8f_rgb');
f2 = net.getParamIndex('fc8b_rgb');
f3 = net.getParamIndex('fc8f_d');
f4 = net.getParamIndex('fc8b_d');

f_rgb = net1.getParam(net1.layers(idx).params);
f_d = net2.getParam(net2.layers(idx).params);

net.params(f1) = f_rgb(1);
net.params(f1).name = 'fc8f_rgb';
net.params(f2) = f_rgb(2);
net.params(f2).name = 'fc8b_rgb';
net.params(f3) = f_d(1);
net.params(f3).name = 'fc8f_d';
net.params(f4) = f_d(2);
net.params(f4).name = 'fc8b_d';

% deconv32 Layer
filters = single(bilinear_u(64, 40, 40)) ;
net.addLayer('deconv32_rgb', ...
  dagnn.ConvTranspose(...
  'size', size(filters), ...
  'upsample', 32, ...
  'crop', [16 16 16 16], ...
  ...
  'hasBias', false, ...
    'opts', {}), ...
  'rgb39', 'rgb46', 'deconvf32_rgb') ;

f = net.getParamIndex('deconvf32_rgb') ;
net.params(f).value = filters ;
net.params(f).learningRate = 0 ;
net.params(f).weightDecay = 1 ;

filters = single(bilinear_u(64, 40, 40)) ;
net.addLayer('deconv32_d', ...
  dagnn.ConvTranspose(...
  'size', size(filters), ...
  'upsample', 32, ...
  'crop', [16 16 16 16], ...
...
f = net.getParamIndex('deconvf32_d') ;
net.params(f).value = filters ;
net.params(f).learningRate = 0 ;
net.params(f).weightDecay = 1 ;
...
net.addLayer('fuse_decision', fuseblock, {'rgb46','d46'}, {'fusion_1'}, {});

% fc layer after fusion
dfuse_conv1 = dagnn.Conv('size', [1 1 80 40], 'hasBias', true);
net.addLayer('fc1_fus', dfuse_conv1, {'fusion_1'}, {'prediction_decision'}, {'filters_f1', 'biases_f1'});
...
% fuse the RGB and D layers in feature layer
fuseblock = dagnn.Concat('dim', 3);
net.addLayer('fusion_feature', fuseblock, {'rgb38','d38'}, {'fuse_feature'}, {});

% first conv layer after feature fusion
ffuse_conv1 = dagnn.Conv('size', [1 1 8192 4096], 'hasBias', true);
net.addLayer('fc1_fusion', ffuse_conv1, {'fuse_feature'}, {'fusion_2'}, {'ffilters_f1', 'fbiases_f1'});
...
% first deconv layer after feature fusion
fuse_filters = single(bilinear_u(64, 40, 40)) ;
fuse_deconv_block = dagnn.ConvTranspose(...
  ...
  'hasBias', false, ...
    'opts', {});                   
net.addLayer('deconv32', fuse_deconv_block, 'fusion_5', 'prediction_feature', 'deconvf_32s');

f = net.getParamIndex('deconvf_32s') ;
net.params(f).value = fuse_filters ;
net.params(f).learningRate = 0 ;
net.params(f).weightDecay = 1 ;
net.vars(net.getVarIndex('prediction_feature')).precious = 1 ;

%------------------------------------------------
%-------------------final fusion-----------------
%------------------------------------------------

final_fuse_block = dagnn.Concat('dim',3);
net.addLayer('final_fusion',final_fuse_block,{'prediction_feature','prediction_decision'},{'fuse_prediction'},{});

% fc layer to modify the third dimention
final_conv1 = dagnn.Conv('size', [1 1 80 40], 'hasBias', true);
net.addLayer('final_fc', final_conv1, {'fuse_prediction'}, {'prediction'}, {'filters_final', 'biases_final'});
